...
export class AreaDetector {
    ...
    private stayCallbacks: Map<
        string,
        { duration: number; callback: () => void; triggered: boolean }
    > = new Map();
    ...
    update(cartPosition: THREE.Vector3) {
        for (const [id, zone] of this.zones) {
            ...
            if (isInside) {
                ...
                if (
                    stayCallback &&
                    !stayCallback.triggered &&
                    timeInArea >= stayCallback.duration
                ) {
