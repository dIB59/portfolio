    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-card/80 backdrop-blur-sm rounded-lg p-4 border text-center">
          ...
          <div className="text-sm text-muted-foreground">Confident</div>
        </div>
        <div className="bg-card/80 backdrop-blur-sm rounded-lg p-4 border text-center">
        ...
        <div className="bg-card/80 backdrop-blur-sm rounded-lg p-4 border text-center">
          ...
          <div className="text-sm text-muted-foreground">Struggled</div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-4 mb-6 items-center">
        ...
        <Select value={filterType} onValueChange={(v) => setFilterType(v as ProblemType | "all")}>
        ...
        <Select value={filterConfidence} onValueChange={(v) => setFilterConfidence(v as Confidence | "all")}>
          <SelectTrigger className="w-40">
            ...
            <thead>
              <tr className="border-b bg-muted/50">
                <th className="text-left p-4 font-semibold text-foreground">
                  <button onClick={() => toggleSort("name")} className="hover:text-foreground/80 transition-colors">
                    Problem
                    <SortIcon field="name" />
                  </button>
                </th>
                ...
                <th className="text-left p-4 font-semibold text-foreground">
                  <button onClick={() => toggleSort("date")} className="hover:text-foreground/80 transition-colors">
                    Date
                    <SortIcon field="date" />
                  </button>
                </th>
                <th className="text-left p-4 font-semibold text-foreground">Got Stuck On</th>
            ...
            <tbody>
              {filteredAndSortedProblems.map((problem, index) => (
                ...
                >
                  <td className="p-4">
                    <div className="font-medium text-foreground">
                      {problem.problemNumber && (
                        <span className="text-muted-foreground font-normal">#{problem.problemNumber} </span>
                      )}
                      {problem.name}
                    </div>
                    {problem.notes && (
                      <div className="text-xs text-muted-foreground mt-1 truncate max-w-xs">{problem.notes}</div>
                    )}
                  </td>
                  <td className="p-4">
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-2">
                      <div className={`w-3 h-3 rounded-full ${confidenceColors[problem.confidence]}`} />
                      <span className="text-sm text-muted-foreground">{confidenceLabels[problem.confidence]}</span>
                    </div>
                  </td>
                  <td className="p-4 text-sm text-muted-foreground">
                    {new Date(problem.solvedDate).toLocaleDateString()}
                  </td>
                  <td className="p-4 text-sm text-muted-foreground max-w-xs truncate">{problem.stuckOn || "-"}</td>
                  {isAdmin && (
                    <td className="p-4">
                      <button
              ...
              {filteredAndSortedProblems.length === 0 && (
                <tr>
                  <td colSpan={isAdmin ? 6 : 5} className="p-8 text-center text-muted-foreground">
                    {problems.length === 0 ? "No problems tracked yet." : "No problems match the current filters."}
                  </td>
                </tr>
              )}
            </tbody>
