    ...
    useFrame((_, delta) => {
        ...
        if (isMoving) {
            ...
            while (spawnTimer.current >= 1) {
                ...
                nextParticle.current =
                    (nextParticle.current + 1) % PARTICLE_COUNT;

                const wheelOffsets = [
                    {
                        x: 0.7,
                        z: -0.9,
                        weight: carState.isDrifting ? 0.4 : 0.3,
                    },
                    {
                        x: -0.7,
                        z: -0.9,
                ...
                p.position.set(
                    carState.position.x +
                        (wheel.x + randX) * cosDir -
                        (wheel.z + randZ) * sinDir,
                    0.05 + Math.random() * 0.15,
                    carState.position.z +
                        (wheel.x + randX) * sinDir +
                        (wheel.z + randZ) * cosDir,
                );
                ...
                p.velocity.set(
                    (Math.random() - 0.5) * spreadAmount -
                        carState.velocity.x * 0.4,
                    Math.random() * upAmount + 2,
                    (Math.random() - 0.5) * spreadAmount -
                        carState.velocity.z * 0.4,
                );

                p.maxLifetime = carState.isDrifting ? 4 : 3;
                p.lifetime = p.maxLifetime;
                p.scale = carState.isDrifting
                    ? 1.5 + Math.random() * 1
                    : 0.8 + Math.random() * 0.6;
            }
        }
        ...
        for (let i = 0; i < PARTICLE_COUNT; i++) {
            ...
            if (p.lifetime > 0) {
                ...
                dummy.scale.setScalar(Math.max(0.001, currentScale * 0.3));
                dummy.updateMatrix();
