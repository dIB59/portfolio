...
import { Plus, Trash2, Edit2, X, ChevronUp, Upload, ImageIcon, Loader2 } from "lucide-react"
...
import { getProjects, addProject, updateProject, deleteProject } from "@/lib/supabase/projects"
    ...
    <div className="space-y-6">
      ...
      <AnimatePresence>
        {showAddForm && (
          ...
          >
            <ProjectForm onSubmit={handleAdd} onCancel={() => setShowAddForm(false)} />
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="space-y-4">
        ...
        {projects.map((project) => (
          ...
          >
            {editingId === project.id ? (
              <ProjectForm
                initialData={project}
                onSubmit={(updates) => handleUpdate(project.id, updates)}
              ...
              <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-1 mt-2">
                      {project.techStack.slice(0, 4).map((tech) => (
                        <span key={tech} className="text-xs bg-muted text-muted-foreground px-2 py-0.5 rounded">
                          {tech}
                        </span>
                      ))}
                ...
                <div className="flex gap-2 shrink-0">
                  <Button variant="ghost" size="icon" onClick={() => setEditingId(project.id)} className="h-8 w-8">
        ...
        {projects.length === 0 && (
          <p className="text-center text-muted-foreground py-8">No projects yet. Add your first project above!</p>
        )}
      </div>
    </div>
  )
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-card/80 backdrop-blur-sm rounded-xl p-6 border space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold text-foreground">{initialData ? "Edit Project" : "Add New Project"}</h3>
