  ...
  useEffect(() => {
    zones.forEach((zone) => {
      areaDetector.addZone({
        ...
        radius: zone.radius,
        onStay: (id, time) => {
          setZoneProgress((prev) => ({ ...prev, [id]: Math.min(time / zone.stayDuration, 1) }))
        },
        onExit: (id) => {
          ...
          <group key={zone.id} position={zone.position.toArray()}>
            ...
            <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.02, 0]}>
              ...
              <meshStandardMaterial
                color={isCompleted ? "#98d4a0" : zone.project.color}
                transparent
                opacity={0.5}
                flatShading
              />
            ...
            <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.03, 0]}>
              <ringGeometry args={[zone.radius - 0.3, zone.radius, 32]} />
              <meshStandardMaterial color={isCompleted ? "#65bf70" : "#e8a5c8"} transparent opacity={0.8} flatShading />
            </mesh>

            {/* Progress ring */}
            {isActive && (
              <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.04, 0]}>
                <ringGeometry args={[zone.radius - 0.15, zone.radius + 0.15, 48, 1, 0, Math.PI * 2 * progress]} />
                <meshStandardMaterial color="#ffaa44" emissive="#ff8800" emissiveIntensity={0.5} flatShading />
              </mesh>
            )}
            ...
            >
              ...
              <meshStandardMaterial
                color={isCompleted ? "#65bf70" : isActive ? "#ffaa55" : zone.project.color}
                flatShading
                emissive={isCompleted ? "#65bf70" : isActive ? "#ff8800" : zone.project.color}
                emissiveIntensity={isCompleted ? 0.5 : isActive ? 0.6 : 0.2}
              />
            </mesh>
            ...
            {isActive && (
              <>
                ...
                <mesh position={[-2.5 + progress * 2.5, 1.8, 0.06]}>
                  <boxGeometry args={[5 * progress, 0.55, 0.1]} />
                  <meshStandardMaterial color="#ffaa44" emissive="#ff8800" emissiveIntensity={0.4} flatShading />
                </mesh>
              </>
            )}
            ...
            {isCompleted && (
              <group position={[0, 0.15, 0]} rotation={[-Math.PI / 2, 0, 0]}>
                ...
                <mesh position={[0, 0, -0.01]}>
                  ...
                  <meshStandardMaterial color={zone.project.color} flatShading />
                </mesh>
                ...
                <Html position={[0, 0, 0.1]} center style={{ pointerEvents: "auto" }}>
                  <a
            ...
            {isCompleted && <pointLight position={[0, 2, 0]} color="#65bf70" intensity={2} distance={10} />}
          </group>
        )
      })}
    </group>
